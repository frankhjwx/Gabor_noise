...
#                 cv2.imshow('Gabor_Kernel_frequency', spatial_to_frequency(kernel.img))
...
if __name__ == '__main__':
    for i in range(7):
        ...
        kernel = Gabor_Noise(size, kernel_size, point_num, k, a, F_0, omega_0, anisotropic)
        #print(kernel.img)
        cv2.imwrite('dst/spacial/'+str(i)+'.png', (kernel.img/2+0.5)*255, [int(cv2.IMWRITE_PNG_COMPRESSION), 3])
        cv2.imwrite('dst/frequency/'+str(i)+'.png', (spatial_to_frequency(kernel.img))*255)
